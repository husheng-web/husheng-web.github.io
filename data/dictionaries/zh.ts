import type { Dictionary } from "@/types/content";

export const zhDictionary = {
  navigation: {
    brandName: "胡昇",
    brandRole: "AI 产品设计师",
    home: "首页",
    projects: "项目",
    about: "关于",
    resume: "简历",
    language: "英文",
    menu: "打开导航菜单",
    closeMenu: "关闭导航菜单",
    menuButton: "菜单",
    closeButton: "关闭",
    skipToContent: "跳至主要内容",
  },
  shell: {
    contentPending: "[内容待补充]",
    comingSoon: "即将更新",
    footerLabel: "作品集系统基础已建立",
    notFoundTitle: "页面不存在",
    notFoundBody: "该页面尚未建立，或地址已发生变化。",
    returnHome: "返回首页",
    allRightsReserved: "保留所有权利",
    backToTop: "返回顶部",
    designSystem: "设计系统",
    archive: "项目档案",
    statusReady: "内容完整",
    statusPartial: "内容部分就绪",
    statusContentPending: "内容待补充",
    statusTranslationPending: "翻译待补充",
  },
  home: {
    heroLabel: "产品作品集",
    heroTitle: "设计并构建 AI 产品与智能服务系统",
    heroSupporting:
      "你好，我是胡昇，一名 AI 产品设计师。我关注如何将复杂的 AI 能力，转化为清晰、易用，并能够创造真实价值的产品体验。",
    heroSecondary: "产品与服务设计",
    viewSelectedWork: "查看我的项目",
    sectionLabels: {
      selectedWorks: "精选项目",
      howIBuild: "构建方法",
      capabilities: "能力框架",
      proof: "实践证明",
      about: "关于我",
      contact: "下一步",
    },
    sectionTitles: {
      selectedWorks: "项目档案",
      howIBuild: "从问题到可运行产品",
      capabilities: "从洞察到交付的产品能力",
      proof: "实践记录",
      about: "AI 产品构建者",
      contact: "继续了解这套作品集系统",
    },
    selectedWorksDescription: "四个重点项目，项目内容会在资料确认后逐步补充。",
    caseStudyInProgress: "案例整理中",
    previewNoMedia: "尚无已确认媒体",
    buildSteps: [
      { index: "01", title: "发现", description: "真实问题 / 用户 / 场景" },
      { index: "02", title: "定义", description: "产品判断 / 价值 / 范围" },
      { index: "03", title: "结构化", description: "服务系统 / 信息架构 / 流程" },
      {
        index: "04",
        title: "智能化",
        description: "AI 工作流 / 智能体 / 模型",
      },
      { index: "05", title: "构建", description: "原型 / 开发 / 集成" },
      { index: "06", title: "验证", description: "测试 / 反馈 / 迭代" },
    ],
    capabilityGroups: [
      { title: "产品与策略" },
      { title: "体验与系统" },
      { title: "AI 产品" },
      { title: "构建与沟通" },
    ],
    proofPending: "",
    aboutCopy: "产品与服务设计背景，关注 AI 产品与智能服务系统。",
    viewAbout: "了解更多",
    contactPending: "",
    viewProjects: "浏览项目",
    viewResume: "查看简历",
  },
  projects: {
    heroLabel: "项目档案",
    heroTitle: "项目档案",
    heroDescription: "记录 AI 产品、数字体验、服务系统与实体产品实践。",
    featuredLabel: "重点项目",
    featuredTitle: "精选项目",
    featuredDescription: "当前优先整理的四个项目。",
    indexLabel: "完整索引",
    indexTitle: "全部项目",
    indexDescription: "按项目编号整理的完整作品档案。",
    indexUniformPendingNote: "当前项目案例均在整理中。",
    projectCountLabel: "个项目",
    caseStudyInProgress: "案例整理中",
    viewCase: "查看案例",
    viewProgress: "查看阶段内容",
    noApprovedMedia: "媒体待补充",
  },
} satisfies Dictionary;
